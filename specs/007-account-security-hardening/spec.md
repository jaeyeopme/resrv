...
**Created**: 2026-05-26
**Status**: Draft
**Input**: User description: "Harden account security for production readiness by adding sign-in throttling, password reset through email after repeated failed sign-ins, and request-time active account/business membership validation without changing the account-scoped token model."
...
**Independent Test**: Can be tested by attempting repeated failed sign-ins for the same account and confirming that normal attempts are accepted until the fifth failure and then require password reset through email.
...
### User Story 2 - Require Password Reset After Suspicious Failures (Priority: P2)

As an account owner, I need password reset through my registered email after suspicious failed sign-in attempts so that an attacker cannot keep guessing indefinitely without proving access to my email.

**Why this priority**: Account-level password reset challenge protects specific accounts even when attempts come from changing caller contexts.
...
- **PasswordResetChallenge**: Account protection state that prevents password sign-in after 5 failed attempts until the account owner resets the password through a delivered email link.
...
- **SC-001**: 100% of accounts with 5 failed password sign-in attempts require password reset before another password sign-in can succeed.
- **SC-002**: 100% of accounts requiring password reset are prevented from signing in with any password until password reset succeeds.
